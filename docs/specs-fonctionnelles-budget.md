...
| **Rappel de saisie** (`NOT-03`) et **notifications de récurrentes** | Les deux types existent dans le modèle `notifications` ; le centre de notifications ne leur donne aucune formulation, donc ils ne s'affichent jamais. **La mécanique de comptage, elle, existe depuis la PR 5/7** : la view `entry_streaks` rend la longueur de la série et le jour où elle s'arrête, et un cron peut la lire telle quelle. | Le N de « aucune saisie depuis N jours », sa valeur par défaut, et son rapport à la série : `NOT-03` doit **lire `entry_streaks`** plutôt que compter une seconde fois, sinon les deux se contrediront un jour — c'est la question que cette ligne posait, et la moitié qui reste ouverte. Reste aussi le fuseau : la view s'arrête au dernier jour saisi et c'est le client qui le compare à sa date locale, or un cron n'a pas de client. |
| **Fusionner deux catégories** (`CAT-02`) | Le renommage et l'apparence sont livrés à la PR 6/7 : les deux écrans de listes offrent désormais une correction, dans une feuille, du nom, de la nature, du parent et de l'ornement — et du type d'un compte, donc de son icône, qui ne se choisit pas. Renommer ne rejoue rien, et c'est mesuré plutôt que lu : une transaction pointe sur sa catégorie par relation, et l'historique affiche le nouveau nom sans que rien n'ait été réécrit. La fusion, elle, déplace des lignes — transactions, enveloppes de tous les mois passés, sous-catégories — et n'a pas été commencée. | Ce que deviennent deux enveloppes du même mois posées sur les deux catégories fusionnées : la somme des plafonds, ou celui de la survivante ; si les alertes déjà émises sur la disparue restent en historique ou tombent avec elle ; si l'opération est réversible, sachant que rien ne conserve d'où venait chaque ligne. |
| **La profondeur de l'arborescence des catégories** | Deux niveaux, et c'est l'écran qui le tient, pas le schéma : le formulaire de correction n'offre aucun parent à une catégorie qui a déjà des enfants, faute de quoi elle disparaîtrait de la liste — celle-ci dessine les racines puis les enfants de chacune. `categories.parent` accepterait pourtant l'écriture, et l'API la laisserait passer. | Si un troisième niveau a un sens pour un budget domestique ; ou, à défaut, si la contrainte descend dans un hook serveur — seul endroit où une écriture faite hors de cet écran la respecterait aussi.|
...
| **Effacer un montant d'un geste** | Le pavé ajoute et ne remplace jamais. Sur la feuille de correction le champ arrive prérempli, donc passer de 20 000 à 25 000 demande cinq ⌫ puis quatre chiffres — mesuré le 04/09/2026 : champ à `20000`, appui sur `5`, résultat `200005`. Le clavier du système offrait « tout sélectionner » ; `inputmode="none"` le retire. Ce n'est pas une impasse — l'appui long ouvre encore les poignées de sélection sans clavier — mais c'est plus long et moins évident, et sur le seul chemin où le champ arrive rempli. Écarté de la PR 4/7 parce qu'aucune des options n'est gratuite. | Laquelle : une touche « C », qui déséquilibre une grille de trois colonnes ; la place de `000`, qui vaut trop cher en XOF pour la lui céder ; ou un bouton « Tout effacer » à côté de la ligne d'état, qui ajoute un contrôle à un écran visant dix secondes. Et si la réponse doit valoir aussi pour le virement, où le champ arrive toujours vide — auquel cas le contrôle serait là sans rien à effacer. |
...
| **Conseil du mois** (`RAP-08`) | Suppose `RAP-03`, la comparaison d'une catégorie mois par mois, qui est reportée elle-même. Le conseil livré à la PR 5/7 est celui de `RAP-07` : il ne regarde que le mois courant — reste à vivre et jours restants — et ne rapproche rien. | Sur quel poste le rapprochement se fait quand plusieurs ont bougé ; ce que « mieux » veut dire pour une catégorie où l'on a moins dépensé parce qu'on a moins vécu ; et si un mois incomplet peut être comparé à un mois plein. |
...
| **La longueur de l'écran du compte** | Sept groupes empilés : profil, apparence, mot de passe, adresse, double authentification, données, fermeture. Mesuré le 06/09/2026 à 390 px : **2 013 px de haut**, contre 1 635 avant la PR 7/7 — les cartes en valent environ 190 et la section « Apparence » 188. Les cartes règlent le vrai problème, qui était sept formulaires sans regroupement, mais elles ne raccourcissent rien. Écarté de la refonte : replier les formulaires d'identifiants est un changement de comportement, que celle-ci s'est interdit. | Si les groupes se replient — et lesquels s'ouvrent d'emblée, sachant que le seul qu'on vient chercher varie —, ou si l'écran se scinde en deux destinations, auquel cas il faut décider laquelle porte l'entrée « Mon compte » de la navigation. Et où va « Fermer mon compte » dans les deux cas : au fond d'un écran long, elle est loin de tout, ce qui est exactement ce qu'on veut d'elle. |
