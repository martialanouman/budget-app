...
- **Le pavé ne se replie pas au focus, et c'est une correction.** La première version l'ouvrait au focus du groupe ; mesuré, cela n'achetait aucune place — la catégorie est sous le pavé dans les deux cas, et il faut l'atteindre pour qu'il se replie — tout en rendant le montant inatteignable tant que rien n'a focalisé le champ. Or rien ne le fait : **`showModal()` donne le focus au bouton « Fermer »**, et React ne rend pas d'attribut `autofocus` que le `<dialog>` préférerait. L'`autoFocus` du champ « Montant » n'avait donc jamais rien fait depuis l'étape 4.
...
- **Le pavé s'arrête à douze chiffres**, et la borne est sur l'affichage autant que sur l'arithmétique : un nombre qui grandit pousse le champ hors de la feuille sans que rien ne dise pourquoi.
