...
- **La console d'administration n'est pas exposée : Traefik refuse de la router.** La règle exclut `/_` **et** `/api/collections/_superusers/`. Ne bloquer que le premier aurait caché la page en laissant la porte ouverte — la console est une page, ce point d'authentification est la porte, et un attaquant qui connaît l'API ne visite jamais la page. Le conteneur publie son port sur `127.0.0.1` du VPS seulement ; on y accède par `ssh -L 8090:localhost:8090`. Vérifié le 29/08/2026 contre un vrai Traefik : `/` et `/api/health` répondent 200 à travers le proxy, `/_/` et l'authentification superadmin y répondent 404, et les deux répondent 200 par le tunnel.
- **Une règle Traefik se vérifie avant d'être livrée.** La première tentative de banc d'essai a rendu 404 sur tout, y compris `/` — non pas à cause de la règle, mais parce que Traefik ne pouvait pas parler au socket Docker de la machine et n'avait créé aucun routeur. Le provider fichier a permis d'éprouver la règle elle-même. Un 404 généralisé ne dit pas _quelle_ pièce manque.
