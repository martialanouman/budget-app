...
- **Le menu se ferme sur le changement de `pathname`, pas sur le clic.** Le bouton « précédent » du navigateur navigue aussi, et laisserait sinon un `<dialog>` — donc un piège à focus et un fond inerte — au-dessus d'une page que personne ne peut atteindre. La fermeture est un **ajustement pendant le rendu** avec son `lastSeenPath` : le compilateur React refuse un `setState` dans un effet, le même mur que le brouillon du champ « Nom », et la même réponse.
