...
- **`icon` et `color` peuvent être vides et le resteront** : rien ne rétro-remplit les lignes antérieures à la migration `1788000000`. Toute lecture passe donc par `hueClassOf` / `iconOf`, qui dérivent du nom — une pastille incolore dans une colonne de pastilles colorées se lit comme un défaut, pas comme un défaut de choix.
